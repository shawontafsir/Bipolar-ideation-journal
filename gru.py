...
class GRU(nn.Module):
    def __init__(
        ...
        self.class_name = f'{"Bi" if is_bidirectional else ""}GRU{"WithAttention" if has_attention else ""}With{embedding_type}Embeddings'
        ...
        if self.embedding_type == 'bert':
            ...
            self.embedding = _bert_model.embeddings
    ...
    def forward(self, input_ids):
        ...
        gru_outputs, _ = self.gru(embeddings)

        # Apply attention mechanism
        if self.has_attention:
            weighted_output = self.attention(gru_outputs)
            fc1_input = self.attention_dropout(weighted_output)
        else:
            # Take the output of the last time step
            fc1_input = gru_outputs[:, -1, :]
